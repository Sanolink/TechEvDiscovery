//priority: 10000
/* 
 _____                                             _____ 
( ___ ) © SanoLink 2024/2026. All rights reserved.( ___ )
 |   |~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|   | 
 |   |  _____         _     _____        ______    |   | 
 |   | |_   _|__  ___| |__ | ____|_   __ \ \ \ \   |   | 
 |   |   | |/ _ \/ __| '_ \|  _| \ \ / /  \ \ \ \  |   | 
 |   |   | |  __/ (__| | | | |___ \ V /   / / / /  |   | 
 |   |  _|_|\___|\___|_| |_|_____| \_/   /_/_/_/   |   | 
 |   | |  _ \(_)___  ___ _____   _____ _ __ _   _  |   | 
 |   | | | | | / __|/ __/ _ \ \ / / _ \ '__| | | | |   | 
 |   | | |_| | \__ \ (_| (_) \ V /  __/ |  | |_| | |   | 
 |   | |____/|_|___/\___\___/ \_/ \___|_|   \__, | |   | 
 |   |                                      |___/  |   | 
 |___|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|___| 
(_____)         Last Modification : 1.5.2         (_____)

*/

const $JsonIO = Java.loadClass('dev.latvian.mods.kubejs.util.JsonIO')
const $Platform = Java.loadClass('dev.architectury.platform.Platform')

global.TechEvItems = [
    'custommachinery:custom_machine_item',
    'techev_additions:techevium',
    'techev_additions:tellurite',
    'techev_additions:chromatic',
    'techev_additions:evolium',
    'techev_additions:raw_auricargentum',
    'techev_additions:raw_cuperzinate',
    'techev_additions:raw_leadosnite',
    'techev_additions:raw_uratinumal',
    'techev_additions:raw_auricargentum_block',
    'techev_additions:raw_cuperzinate_block',
    'techev_additions:raw_leadosnite_block',
    'techev_additions:raw_uratinumal_block',
    'techev_additions:villager_bell',
    'techev_additions:svartalfheim_portal',
    'mysticalagriculture:alfsteel_essence',
    'mysticalagriculture:calorite_essence',
    'mysticalagriculture:chocolate_essence',
    'mysticalagriculture:cinnabar_essence',
    'mysticalagriculture:desh_essence',
    'mysticalagriculture:destabilized_redstone_essence',
    'mysticalagriculture:dragonsteel_fire_essence',
    'mysticalagriculture:dragonsteel_ice_essence',
    'mysticalagriculture:dragonsteel_lightning_essence',
    'mysticalagriculture:echo_essence',
    'mysticalagriculture:energized_glowstone_essence',
    'mysticalagriculture:iesnium_essence',
    'mysticalagriculture:nephryx_essence',
    'mysticalagriculture:oil_essence',
    'mysticalagriculture:ostrum_essence',
    'mysticalagriculture:peridot_essence',
    'mysticalagriculture:pink_slime_essence',
    'mysticalagriculture:plastic_essence',
    'mysticalagriculture:resonant_ender_essence',
    'mysticalagriculture:soul_steel_essence',
    'mysticalagriculture:source_gem_essence',
    'mysticalagriculture:tea_essence',
    'mysticalagriculture:wasted_radioactive_essence',
    'mysticalagriculture:alfsteel_seeds',
    'mysticalagriculture:awakened_supremium_seeds',
    'mysticalagriculture:calorite_seeds',
    'mysticalagriculture:chocolate_seeds',
    'mysticalagriculture:cinnabar_seeds',
    'mysticalagriculture:desh_seeds',
    'mysticalagriculture:destabilized_redstone_seeds',
    'mysticalagriculture:dragonsteel_fire_seeds',
    'mysticalagriculture:dragonsteel_ice_seeds',
    'mysticalagriculture:dragonsteel_lightning_seeds',
    'mysticalagriculture:echo_seeds',
    'mysticalagriculture:energized_glowstone_seeds',
    'mysticalagriculture:iesnium_seeds',
    'mysticalagriculture:imperium_seeds',
    'mysticalagriculture:insanium_seeds',
    'mysticalagriculture:nephryx_seeds',
    'mysticalagriculture:oil_seeds',
    'mysticalagriculture:ostrum_seeds',
    'mysticalagriculture:peridot_seeds',
    'mysticalagriculture:pink_slime_seeds',
    'mysticalagriculture:plastic_seeds',
    'mysticalagriculture:prudentium_seeds',
    'mysticalagriculture:resonant_ender_seeds',
    'mysticalagriculture:soul_steel_seeds',
    'mysticalagriculture:source_gem_seeds',
    'mysticalagriculture:supremium_seeds',
    'mysticalagriculture:tea_seeds',
    'mysticalagriculture:tertium_seeds',
    'mysticalagriculture:ultimate_seeds',
    'mysticalagriculture:wasted_radioactive_seeds'
]
global.TechEvBlocks = []

global.TechEvBlockItems = []

const $Config = $Platform.getConfigFolder()
global.TechEvCreateDecoCoinsMat = JSON.parse(
    String($JsonIO.readString($Config.getFileSystem().getPath($Config.toString(), 'techev_additions-coins.json')))
).map(name => name.toLowerCase().replace(/ /g, '_'))

global.TechEvCreateDecoCoinsMat.forEach(mat => global.TechEvItems.push(`createdeco:${mat}_coinstack`))